...
5. Sett opp port forwarding til 7164 via `k9s`
  1. Logg inn via `nais login` ([se her](https://doc.nais.io/operate/cli/how-to/install/) for hvordan du installerer nais-cli)
  2. Installer k9s med `brew install k9s`
  3. Kjør `kubectl use-context dev-gcp` ([se her](https://kubernetes.io/docs/tasks/tools/install-kubectl-macos/#install-with-homebrew-on-macos) for hvordan du setter opp `kubectl`)
  3. Kjør `k9s`, trykk `ctrl+f` og endre porten til `7164`
6. Nå skal du kunne gjøre kall fra lokal frontend til lokal backend 🎉
